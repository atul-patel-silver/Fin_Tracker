...
import org.keycloak.fedration.model.QueryConfigurations;
import org.keycloak.fedration.model.UserAdapter;
import org.keycloak.fedration.persistence.DataSourceProvider;
import org.keycloak.fedration.persistence.UserRepository;
import org.keycloak.fedration.util.PagingUtil;
...
//        }
        ...
        if (allowDatabaseToOverwriteKeycloak && user instanceof CachedUserModel && (System.currentTimeMillis() - ((CachedUserModel) user).getCacheTimestamp()) > 500) {
          ...
          if (!Objects.equals(user.getUsername(), dbUser.getUsername()) || !Objects.equals(user.getEmail(), dbUser.getEmail())) {
            ((CachedUserModel) user).invalidate();
          }
        }
        log.infov("isValid user credential: userId={0}", user.getId());
